...
# ── Sidebar ───────────────────────────────────────────────────────────────────
BUNDLED_CSV = "imdbdata.csv"  # auto-loads if committed to repo
...
with st.sidebar:
    ...
    if os.path.exists(BUNDLED_CSV):
        ...
        st.markdown("Upload your `imdbdata.csv` to train the model.")
...
# ── TAB 1 — Train & Metrics ───────────────────────────────────────────────────
with tab1:
    # Decide data source: bundled CSV > uploaded file > nothing
    if os.path.exists(BUNDLED_CSV):
    ...
    if data_source is None:
        ...
        try:
            df_raw = pd.read_csv(data_source, encoding="latin1", engine="python",
